...
import {CASE, CASE_STEP_DESCRIBE, PRODUCT, PRODUCT_MODULE} from "../../../case/rmind/statics/refer";

export default (nodeStatus, mindmapHook, historyHook) => {
    const {addChild, addSibling, editNode, deleteNode, toggleChildren, selectNode,addCase,addCaseStep} = mindmapHook;
    ...
    const handleKeyEventWithNode = event => {
        switch (event.key.toUpperCase()) {
            ...
            case 'ENTER':
                ...
                const currentNode = nodeStatus.cur_node_info;
                const parentNode = nodeStatus.cur_node_info.parent;
                ...
                switch (currentNode.type) {
                    ...
                    case PRODUCT_MODULE:
                        break;
                    case CASE:
                        addCase(parentNode.id, parentNode.moduleId, parentNode.productId, parentNode.requireId,window.__data.staffCode);
                        break;
                    case CASE_STEP_DESCRIBE:
                        addCaseStep(parentNode.id, currentNode.moduleId, currentNode.productId, currentNode.requireId,
                            currentNode.caseId, currentNode.version, window.__data.staffCode);
                        break;
                }
                break;
            case 'F2':
