...
import {CASE, CASE_STEP_DESCRIBE, CASE_STEP, PRODUCT_MODULE} from "../statics/refer";
        ...
        },
        {
            ...
            listener: event => {
                ...
                if (target_id !== '' && target_id !== node_id && target_id !== parent_id) {
                    const targetNode = findNode(mindmap, target_id);
                    const currentNode = findNode(mindmap, node_id);
                    if (targetNode === undefined) {
                        return;
                    }
                    //用例节点可以拖动到模块节点，步骤描述节点可以拖动到步骤节点
                    if (((targetNode.type === PRODUCT_MODULE && currentNode.type === CASE)) ||
                        (targetNode.type === CASE_STEP && currentNode.type === CASE_STEP_DESCRIBE)) {
                        mindmapHook.moveNode(node_id, target_id, parent_id, is_sibling, targetNode.productId,
                            targetNode.moduleId, targetNode.caseId, currentNode.type, targetNode.version);
                    }
                }
            }
        },
